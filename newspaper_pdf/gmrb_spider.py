...
通过 HTML 页面抓取光明日报各版面文章，并导出为格式化的 PDF 文件。

数据源特点：
- 通过 HTML 页面解析获取版面和文章数据
- 从首个版面页提取版面链接，再从各版面页提取文章链接
- 版面页的标题目录（.m-title-list）包含版面名称和当版文章链接
- 文章页的标题区（h3/h1/h2）与稿件信息（.m-article-author）结构固定

使用方式：
    python gmrb.py                         # 抓取当天
    python gmrb.py --date 2026-03-10       # 指定日期
"""
...
# 光明日报站点默认根地址
DEFAULT_BASE_URL = "https://epaper.gmw.cn"


class GMRBSpider:
    """光明日报爬虫。

    通过 HTML 页面解析获取报纸版面和文章数据。处理流程：
    1. 从入口页获取最新日期和版面列表
    2. 从各版面页提取文章链接
    3. 从各文章页提取标题、作者和正文
    ...
    # 去除作者字段中"作者："前缀的正则
    _AUTHOR_PREFIX_PATTERN = re.compile(r"^作者[:：]\s*")
    ...
    def resolve_paper_date(self, target_date: str | None) -> str:
        ...
        # 从版面导航入口页的链接中提取日期
        # 链接格式如：202609/06/node_01.html
        entry_url = f"{self.base_url}/gmrb/html/layout/index.html"
        html_text = self._fetch_html(entry_url)
        match = re.search(r"(\d{6})/(\d{2})/node_\d+\.html", html_text)
        if not match:
            raise RuntimeError("未能从光明日报入口页解析出最新版面日期")
    ...
    def _discover_section_urls(self, paper_date: str) -> list[str]:
        ...
        if not section_numbers:
            raise RuntimeError("未能从光明日报版面页提取版面链接")
    ...
    def _extract_section_meta(
        ...
        Returns:
            (版面编号, 版面名称) 元组
        """
        # 标题目录区的 .m-type 由两个 span 组成，形如 "01版:" + "头版"
        section_label = normalize_space(
            _extract_text(soup.select_one(".m-title-list .m-type"))
        )
    ...
    def _extract_article_urls(self, soup: BeautifulSoup, section_url: str) -> list[str]:
        ...
        for anchor in soup.select(".m-title-list a[href]"):
            href = _attr_to_text(anchor.get("href"))
    ...
    def _parse_article(
        ...
        title = normalize_space(_extract_text(soup.select_one("h1")))
        if not title:
        ...
        body_container = soup.select_one(".m-article-main")
    ...
    def _extract_subtitle(self, soup: BeautifulSoup) -> str:
        ...
        """
        lines: list[str] = []
        for node in soup.select("h3"):
            text = normalize_space(node.get_text(" ", strip=True))
            if text:
                lines.append(text)
        subtitle_node = soup.select_one("h2")
        if subtitle_node is not None:
            text = normalize_space(subtitle_node.get_text(" ", strip=True))
            if text:
                lines.append(text)
    ...
    def _extract_author(self, soup: BeautifulSoup) -> str:
        ...
        作者位于 .m-article-author，形如"作者：本报记者 晋浩天"，
        去除"作者："前缀后返回。
        ...
        Returns:
            作者信息，无则为空字符串
        """
        raw_author = normalize_space(
            _extract_text(soup.select_one(".m-article-author"))
        )
        return self._AUTHOR_PREFIX_PATTERN.sub("", raw_author)
    ...
    def _build_node_url(self, paper_date: str, paper_number: str) -> str:
        ...
        return (
            f"{self.base_url}/gmrb/html/layout/{year}{month}/{day}/"
...
def build_argument_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="抓取光明日报当天所有版面文章，并导出为 PDF。"
    )
    ...
    parser.set_defaults(out_dir="output/gmrb")
    return parser
...
def main() -> None:
    ...
    spider = GMRBSpider(base_url=args.base_url)
